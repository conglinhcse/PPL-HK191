...
def main(argv):
    ...
    if len(argv) < 1:
        ...
        if len(argv) < 2:
            ...
            suite = unittest.makeSuite(ASTGenSuite)
            test(suite)
        elif argv[1] == 'ASTGenSuiteKhoi':
            from ASTGenSuiteKhoi import ASTGenSuite
            suite = unittest.makeSuite(ASTGenSuite)
            test(suite)
        elif argv[1] == 'ASTGenSuiteKhoi2':
            from ASTGenSuiteKhoi2 import ASTGenSuite
            suite = unittest.makeSuite(ASTGenSuite)
            test(suite)
        elif argv[1] == 'ASTGenSuiteThuc':
            from ASTGenSuiteThuc import ASTGenSuite
            suite = unittest.makeSuite(ASTGenSuite)
            test(suite)
        else:
            printUsage()
