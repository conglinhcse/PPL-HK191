...
def main(argv):
    ...
    if len(argv) < 1:
        ...
        if len(argv) < 2:
            ...
            test(suite)
        elif argv[1] == 'CheckSuite1H':
            from CheckSuite1H import CheckSuite
            suite = unittest.makeSuite(CheckSuite)
            test(suite)
        elif argv[1] == 'CheckSuiteT':
            from CheckSuiteT import CheckSuite
            suite = unittest.makeSuite(CheckSuite)
            test(suite)
        elif argv[1] == 'CheckSuite2':
            from CheckSuite2 import CheckSuite
            suite = unittest.makeSuite(CheckSuite)
            test(suite)
        elif argv[1] == 'CheckSuiteCD':
            from CheckSuiteCD import CheckSuite
            suite = unittest.makeSuite(CheckSuite)
            test(suite)
        elif argv[1] == 'CheckSuiteT3':
            from CheckSuiteT3 import CheckSuite
            suite = unittest.makeSuite(CheckSuite)
            test(suite)
        elif argv[1] == 'CheckSuiteThuc':
            from CheckSuiteThuc import CheckSuite
            suite = unittest.makeSuite(CheckSuite)
            test(suite)
        elif argv[1] == 'CheckSuiteFinal':
            from CheckSuiteFinal import CheckSuite
            suite = unittest.makeSuite(CheckSuite)
            test(suite)
        elif argv[1] == 'CheckSuiteTMQ':
            from CheckSuiteTMQ import CheckSuite
            suite = unittest.makeSuite(CheckSuite)
            test(suite)
        else:
            printUsage()
